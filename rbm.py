...
class RBM:
    ...
    def fit(self,
            data_x,
            data_val,               #define validation dataset
            n_epoches=10,
            batch_size=10,
            shuffle=True,
            verbose=True):
        ...
        for e in range(n_epoches):
            ...
            epoch_errs_ptr = 0
            epoch_errs_val = np.zeros((n_data_val,))
            ...
            for num_val in range(0,n_data_val):
                epoch_errs_val[num_val]=self.get_err(data_val[num_val,:].reshape(1,-1))

            if verbose:
                err_mean = epoch_errs.mean()
                err_mean_val = epoch_errs_val.mean()
                if self._use_tqdm:
                    self._tqdm.write('Train error: {:.4f}'.format(err_mean))
                    self._tqdm.write('')
                    self._tqdm.write('Validation error: {:.4f}'.format(err_mean_val))
                    self._tqdm.write('')
                else:
                    print('Train error: {:.4f}'.format(err_mean))
                    print('')
                    print('Validation error: {:.4f}'.format(err_mean_val))
                    print('')
                sys.stdout.flush()

            errs = np.hstack([errs, epoch_errs])
            errs_val = np.hstack([errs_val, err_mean_val])

        return errs, errs_val
